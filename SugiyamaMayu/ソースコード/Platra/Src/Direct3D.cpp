...
// M.S 画面分割変更のためのビューポートをセットする関数
//引  数：bool doubleScreen    trueならば2画面表示 
//                             falseならば1画面表示
//戻り値：なし
void CDirect3D::SetViewPort(bool doubleScreen) {
	//ビューポートの設定
	D3D11_VIEWPORT vp[2];
	vp[0].Width = m_dwWindowWidth;
	vp[0].Height = m_dwWindowHeight;
	vp[0].MinDepth = 0.0f;
	vp[0].MaxDepth = 1.0f;
	vp[0].TopLeftX = 0;
	vp[0].TopLeftY = 0;
	// 画面分割したビューポートの設定
	vp[1].Width = m_dwWindowWidth / 2;
	vp[1].Height = m_dwWindowHeight / 2;
	vp[1].MinDepth = 0.0f;
	vp[1].MaxDepth = 1.0f;
	vp[1].TopLeftX = m_dwWindowWidth / 2;
	vp[1].TopLeftY = m_dwWindowHeight / 2;
