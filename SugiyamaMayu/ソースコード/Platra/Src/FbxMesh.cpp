...
CFbxMesh::CFbxMesh(CFbxMeshCtrl* pFbxMeshCtrl)
{
	...
	m_pD3D->CreateShaderResourceViewFromFile(_T("Data/Image/toonPicture.png"), &m_pD3D->m_pToonTexture, 3);   // トゥーンレンダリング用画像の読み込み
...
//
//------------------------------------------------------------------------
void CFbxMesh::Render(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye, int shineFlag)
{
		if (m_nMeshType == 1)
		{
			RenderStatic(mWorld, mView, mProj, vLight, vEye, shineFlag);   // shineFlagの追加 M.S
		}
...
//
//------------------------------------------------------------------------
void CFbxMesh::Render(ANIMATION_STATUS& animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye, int shineFlag)
{
		if (m_nMeshType == 1)
		{
			RenderStatic(mWorld, mView, mProj, vLight, vEye, shineFlag);   // shineFlagの追加 M.S
...
//      const VECTOR3&   vEye                視点ベクトル
//              int   shineFlag              レンダーの指定  M.S
//      shineFlag   0:暗い普通のレンダー、1:明るいトゥーンレンダー、2:明るい透明、3:明るい普通のレンダー 4:暗い透明
//
//	戻り値 HRESULT
//		S_OK	= 正常
//		E_FAIL	= 異常
//
//------------------------------------------------------------------------
void CFbxMesh::RenderStatic(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye, int shineFlag)
{
	// M.S.shineFLag 0:暗い普通のレンダー 1:明るいトゥーンレンダー 2:明るい透明 3:明るい普通のレンダー 4:暗い透明
	// 使用するシェーダーの登録	
	if (shineFlag == 0 || shineFlag == 2 || shineFlag == 3 || shineFlag == 4) {
		// M.S 通常のシェーダー
		m_pD3D->m_pDeviceContext->VSSetShader(m_pShader->m_pFbxNormalStaticMesh_VS, NULL, 0);
		m_pD3D->m_pDeviceContext->PSSetShader(m_pShader->m_pFbxNormalStaticMesh_PS, NULL, 0);
	}
	else if (shineFlag == 1) {
		// M.S トゥーンレンダーのシェーダー
	...
	if (SUCCEEDED(m_pD3D->m_pDeviceContext->Map(m_pShader->m_pConstantBufferWVLED, 0, D3D11_MAP_WRITE_DISCARD, 0, &pData)))
	{
		...
		// ワールド、カメラ行列を渡す
		cb.mWV = XMMatrixTranspose(mWorld * mView);
		//ワールド、カメラ、射影行列を渡す
		cb.mWVP = XMMatrixTranspose(mWorld * mView * mProj);
		//ライト方向を渡す
		if (shineFlag == 1) {
			// M.S 線路がどこからでも光るようにする
			cb.vLightDir = VECTOR4(0, 0, 0, 0);
		}
		else {
		...
		// M.S 暗い普通のレンダー
	    if (shineFlag == 0) {
		cb.vDiffuse = VECTOR4(0.75, 0.75, 0.75, 1.0);
	    }
		// M.S 明るいトゥーンレンダー
		else if (shineFlag == 1) {
			cb.vDiffuse = VECTOR4(2.5, 2.5, 2.5, 1.0);
		}
		// M.S 明るい透明
		else if(shineFlag == 2){
			cb.vDiffuse = VECTOR4(1.3, 1.3, 1.3, 0.8);
		}
		// M.S 明るい普通のレンダー
		else if (shineFlag == 3) {
			cb.vDiffuse = VECTOR4(1.0, 1.0, 1.0, 1.0);
		}
		// M.S 暗い透明
		else if (shineFlag == 4) {
			cb.vDiffuse = VECTOR4(0.5, 0.5, 0.5, 0.8);
		}
		// 各種情報を渡す。(使っていない)
	...
	//頂点インプットレイアウトをセット
	if (shineFlag == 1) {
		// M.S トゥーンレンダーインプットレイアウトをセット
		m_pD3D->m_pDeviceContext->IASetInputLayout(m_pShader->m_pFbxStaticMesh_VertexLayout);
	}
	else{
		// M.S 通常のレンダーのインプットレイアウトをセット
		m_pD3D->m_pDeviceContext->IASetInputLayout(m_pShader->m_pFbxNormalStaticMesh_VertexLayout);
	}
	//プリミティブ・トポロジーをセット
...
void CFbxMesh::Draw(const int& animNum, const int& frame, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	...
	if (SUCCEEDED(m_pD3D->m_pDeviceContext->Map(m_pShader->m_pConstantBufferWVLED, 0, D3D11_MAP_WRITE_DISCARD, 0, &pData)))
	{
		...
		//カラーを渡す
		cb.vDiffuse = VECTOR4(1.1,1.1,1.1,1);
	...
	for (DWORD mi = 0; mi < m_dwMeshNum; mi++)   // -- 2018.8.1
	{
		...
		SetShaderTexture(_T("PS"), 1, &m_pMeshArray[i].m_pTextureNormal);       // ノーマルテクスチャ
		SetShaderTexture(_T("PS"), 3, &m_pMeshArray[i].m_pTextureSpecular);     // スペキュラテクスチャ
		SetShaderTexture(_T("PS"), 4, &m_pD3D->m_pToonTexture);       // ピクセルシェーダーにトゥーンレンダリング用画像を渡す
